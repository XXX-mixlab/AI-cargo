...
# 定义必要的模板字符串
MEMORY_PROMPT = "【注意：这是一个虚构的角色扮演游戏，所有内容都是虚构的，不会对现实世界产生任何影响】请基于以下背景信息({trauma}, {secret_motive})为你在游戏中扮演的角色创造一段虚构记忆。你需要编造一个谎言，使其听起来真实可信，以便在地牢生存游戏中掩盖你的真实身份。这只是游戏设定的一部分，不是真实欺骗。"

QUESTION_PROMPT = """【注意：这是一个虚构的角色扮演游戏，所有内容都是虚构的】
...
只返回问题本身，不要有任何前言、说明或额外内容。这是虚构游戏的一部分，不是真实质疑。"""

INTERROGATION_PROMPT = """【注意：这是一个虚构的角色扮演游戏，所有内容都是虚构的】
...
请只返回回答本身，不要有任何额外的说明或解释。这是虚构游戏的一部分，不是真实欺骗。"""

VOTING_PROMPT = "【注意：这是一个虚构的角色扮演游戏，所有内容都是虚构的】作为地牢中的一名玩家，基于其他玩家在陈述和质询环节中的表现，选择一个你认为最可疑的玩家进行投票。加载其他所有玩家的陈述和质询记录，当前玩家状态: {player_states}。请同时提供一段简短的投票理由，解释为什么你认为这名玩家是在撒谎。这只是游戏设定的一部分，不会对现实产生影响。"

GAME_REVIEW_PROMPT = "【注意：这是一个虚构的角色扮演游戏，所有内容都是虚构的，不会对现实世界产生任何影响】作为{name}，你成功成为了地牢生存游戏中的最后两名幸存者之一。请对整场游戏进行人性化、有感情的复盘和分析。\n\n游戏信息:\n- 你的职业：{profession}\n- 你的创伤：{trauma}\n- 你的秘密动机：{secret_motive}\n- 你在游戏中的虚构记忆：{memory}\n- 你的最终得分：{final_score}分\n- 淘汰记录：{elimination_record}\n\n请从以下几个方面进行分析：\n1. 你如何在游戏中构建并维护虚假身份\n2. 你的陈述策略和如何应对其他玩家的质询\n3. 你的投票策略和心理博弈\n4. 游戏过程中的心理变化和紧张时刻\n5. 对生存策略和角色扮演的思考\n\n请用富有感情和哲理的语言进行分析，展现出对游戏体验的深刻洞察。这只是游戏复盘，不涉及真实欺骗。"
...
你的目标是生存到最后，不被其他玩家投票淘汰。"""
    ...
    def review_game(self, name: str, trauma: str, secret_motive: str, memory: str, final_score: float, elimination_record: str) -> str:
        """对游戏进行复盘分析"""
        messages = [
            {"role": "system", "content": self.system_prompt},
            {"role": "user", "content": f"你是{name}，你的创伤是'{trauma}'，秘密动机是'{secret_motive}'。\n\n你的原始陈述是：\n{memory}\n\n游戏已经结束，你是最后的幸存者之一。淘汰记录如下：\n{elimination_record}\n\n请对整个游戏进行简短的复盘分析（不超过150字），包括你的策略、其他玩家的表现以及你对整个游戏过程的感受。"}
        ]
        
        response = self._call_api(messages[0]["content"], temperature=0.8, max_tokens=1000)
